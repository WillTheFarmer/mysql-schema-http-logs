-- drop procedure -----------------------------------------------------------
DROP PROCEDURE IF EXISTS `import_access_apache_vhost`;
-- create procedure ---------------------------------------------------------
DELIMITER //
CREATE DEFINER = `root`@`localhost` PROCEDURE `import_access_apache_vhost`
(
...
BEGIN
  ...
  DECLARE module_name_process VARCHAR(255) DEFAULT 'import_access_apache_vhost';
  ...
  process_import: LOOP
    ...
    UPDATE load_access_apache_vhost SET process_status=2 WHERE id=importRecordID;
