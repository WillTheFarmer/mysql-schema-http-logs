-- drop procedure -----------------------------------------------------------
DROP PROCEDURE IF EXISTS `import_access_apache_combined`;
-- create procedure ---------------------------------------------------------
DELIMITER //
CREATE DEFINER = `root`@`localhost` PROCEDURE `import_access_apache_combined`
(
...
BEGIN
  ...
  DECLARE module_name_process VARCHAR(255) DEFAULT 'import_access_apache_combined';
  ...
  -- declare cursor for format load table - All importloadIDs not processed
  ...
  DECLARE combinedStatusFile CURSOR FOR
    ...
    INNER JOIN import_file f
            ON l.importfileid = f.id
         WHERE l.process_status = 1;
  -- declare cursor for format load table - single importLoadID
  ...
  END IF;
  -- in_importLoadID has been set to zero or number passed.
  -- to make sure parameter passed is INSERTED before procedure was called
  ...
  END IF;
  -- if procedure call from Python App importload_ID > 0 and import_process record already inserted.
  -- importServerProcessID() SETs importserverid to serverid and return ID
  -- if importload_ID = 0 import_process INSERT and return ID
  SET importProcessID = importServerProcessID( module_name_process, in_processName, importLoad_ID);
  -- check if files from multiple main:process_files
  ...
  process_import: LOOP
    ...
    UPDATE load_access_apache_combined SET process_status=2 WHERE id=importRecordID;
