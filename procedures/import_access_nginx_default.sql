-- drop procedure -----------------------------------------------------------
DROP PROCEDURE IF EXISTS `import_access_nginx_default`;
-- create procedure ---------------------------------------------------------
DELIMITER //
CREATE DEFINER = `root`@`localhost` PROCEDURE `import_access_nginx_default`
(
...
BEGIN
  ...
  DECLARE module_name_process VARCHAR(255) DEFAULT 'import_access_nginx_default';
...
--  SET importProcessID = importServerProcessID('access_import', in_processName);
  ...
  process_import: LOOP
    ...
    UPDATE load_access_nginx_default SET process_status=2 WHERE id=importRecordID;
