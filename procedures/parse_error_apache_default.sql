...
BEGIN
  ...
  DECLARE module_name_process VARCHAR(255) DEFAULT 'parse_error_apache_default';
...
INNER JOIN import_load il
  ...
  process_parse: LOOP
    ...
    UPDATE load_error_apache_default
...
--     WHERE id = importRecordID AND LOCATE(' ,', httpmessage)>0 AND server_name IS NULL;
    ...
    UPDATE load_error_apache_default
     ...
     WHERE id=importRecordID;
    UPDATE load_error_apache_default SET process_status=1 WHERE id=importRecordID;
